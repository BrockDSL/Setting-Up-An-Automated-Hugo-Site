...
# Building the Site
The first step is to open a command line interface on your machine, navigate to a folder that has access to both Hugo and Git, and then build your base Hugo site using the command below. 
...
Read through the template provided to get an idea of how to set up an action.  Below is an example action that works for the Blist theme with notes explaining each piece.  Notice the task that installs dependencies.  Make sure whatever dependencies your theme needs are installed before the Build task. 
