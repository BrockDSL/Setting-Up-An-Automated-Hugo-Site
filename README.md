...
This is done by having 2 GitHub repositories.  One for the front-end that just hosts the GitHub Pages content, and one for the back-end which houses the Hugo builder and theme files.  The back-end site also has a GitHub action in place that watches for commits and then rebuilds the site and pushes the new version to the front-end whenever a change is made. 
...
# Building the Site
The first step is to open a command line interface on your machine, navigate to a folder that has access to both Hugo and Git, and then build your base Hugo site using the command below (Replacing NameOfSite with the name you want to use for your site with no spaces or special characters). 

    hugo new site NameOfSite

Now that you have the Hugo builder prepped, move into the site folder using the cd command (`cd NameOfSite`), initialize the folder as a git repository (`git init`), and add your theme.  In most cases, this will be done by adding the theme as a git submodule and the code to do this should be listed on the webpage for the theme.  An example of what that code should look like can be seen below (the examples shows adding the “Blist” theme).

    git submodule add https://github.com/apvarun/blist-hugo-theme.git themes/blist

Once the theme is installed as a submodule, look at the theme’s documentation (found on the themes instruction page) to see if it requires configuration.  Very basic themes might work simply by adding it as a submodule but most of the more complex themes require additional steps.  In the case of the Blist theme used in this example, the documentation walks you through how to set up some Node JS features and a css manager called PostCSS that are needed to use the theme.  Once you have finished setting up your theme you are ready to move on.

And just like that you have a Hugo site with a theme!  Now we need to adjust the config.toml file to reflect the correct url for your site.  Open up the config.toml file in the root folder of your site (I use Notepad++ for this but you can use your preferred plain-text editor) and update the baseurl field to reflect your sites location.  Use the format below by replacing GITHUBUSERNAME with your username or the name of the GitHub organization that the site is in and replace NAMEOFSITE with the name of your front-end site. 
...
With that your site should be uploaded to GitHub.  Next you need to make it live on GitHub Pages by going to your front-end site on GitHub, clicking on Settings, clicking “Pages” in the left menu, then adjusting the dropdown under the “Source” header to “master” (or "main" depending on how you set up your branches) and clicking the save button. 

After a minute or two your front-end site will be live at the link shown on the screen.  At this point your front-end site is ready to go and you should never need to manually interact with this repository again! 
...
- COPY YOUR TOKEN TO A SECURE LOCATION NOW as this is the only time you will ever be able to see your token.  If you lose it you will need to make a new one.

Once you have a personal access token, go back to your back-end repository and open the repository settings (from the top menu).  From the left-hand menu, click “Secrets” and then “Actions”.  This is where you can create encrypted variables for use in your GitHub actions.  Keep in mind that anyone with collaborator or higher access to your repository can use these variables. 
...
# Building the Action
Now that we have our permissions variable, it is time to write the action that will handle our automation.  To begin, go to the Actions section of your back-end repository.  Since we are just making a workflow, we will click the “set up a workflow yourself” option near the top of the page which will create a new workflow file for us and open the editor. 

Read through the template provided to get an idea of how to set up a workflow.  Below is an example action that works for the Blist theme with notes explaining each piece.  Notice the task that installs dependencies.  Make sure whatever dependencies your theme needs are installed before the Build task. 
...
#This makes the workflow trigger on any push to the master branch of the repository (this includes commits done online!)
on:
  push:
    branches: [ master ]

#This is the list of tasks that the workflow will execute.  
#They will be executed in the order listed in an Ubuntu environment built by the "runs-on" command
jobs:
  build:
    runs-on: ubuntu-latest

    steps:
      #This action checks out the git repository so it can be used by your workflow
      - uses: actions/checkout@v2
        with:
          submodules: true 
          fetch-depth: 1   
      #This task sets up hugo in the workflow's environment
      - name: Setup Hugo
...
Once you have your workflow built, commit it and it will run automatically.
...
GIthub Marketplace homepage for Actions for Hugo: [https://github.com/marketplace/actions/hugo-setup](https://github.com/marketplace/actions/hugo-setup)  
